...
public class Zombie : LivingEntity
{
    ...
    private Animator zombieAnimator; // 애니메이트 컴포넌트
    ...
    private void Awake()
    {
        ...
        // 렌더러 컴포넌트는 자식 게임 오브젝트에 있음 -> GetComponentInChildren()
    ...
    private void Start()
    {
        // 게임 오브젝트ㅡ 호라성화와 동시에 AI의 추적 루틴 시작
    ...
    public override void OnDamage(float damage, Vector3 hitPoint, Vector3 hitNormal)
    {
        ...
        // LivingEntity 의 OnDamage()를 실행하여 데미지 적용
    ...
    {
        // LivingEntity 의 Die()를 실행하여 기본 사망 처리 실행
        base.Die();
    
        // 다른 AI를 방해하지 않도록 잣니의 모든 콜라이더를 비활성화
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (!dead && Time.time >= lastAttackTime + timeBetAttack)
        {
            ...
            // 상대방의 LivingEntity 가 자신의 추적대상이라면 공격 실행
            if (attackTarget != null && attackTarget == targetEntity)
            {
                // 최근 공격 시간 갱신
                lastAttackTime = Time.time;
                
                // 상대방의 피격 위치와 방향을 근삿값으로 계산
